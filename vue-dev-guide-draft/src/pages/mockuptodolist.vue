...
const updateTodo = ({ id }) => {
  objectUpdateTodo.value = id
}

const todoDoneCount = computed(() => {
  return todoList.filter(object => object.done).length
})
</script>
    ...
    <div class="flex px-mx-auto">
      ...
      <div class="second-wrap relative">
        <div class="header flex font-mono justify-center justify-between">
          ...
          <button class="icon-btn mx-2 !outline-none mt-1" @click="allDelete(todo)">
            <div i="carbon-checkbox-indeterminate" />
          </button>
        </div>
        <div class="list-wrap font-mono text-left p-5">
          <div class="relative">
            <ul>
              <li v-for="(todo, id) in todoList" :key="`todo-id-${todo.id}`">
                <el-input v-if="objectUpdateTodo === todo.id" ref="objectTodoInput" v-model="todo.todo" class="mt-1" @keyup.enter="updateTodo" />
                <el-checkbox v-else v-model="todo.done" :label="todo.todo" size="large" />
                <button class="icon-btn mx-2 !outline-none absolute right-8 mt-3" @click="updateTodo(todo)">
                  <div i="carbon-checkmark" />
                </button>
                <button class="icon-btn mx-2 !outline-none absolute right-0 mt-3" @click="deleteTodo(id)">
